...
class Test_BookmarkViewset(TestCase):
    ...
    def setUp(self):
        ...
        bookmark2.save()
        bookmark3 = Bookmark(link="http://www.bbc.cp.uk/")
        bookmark3.save()
    ...
    def test_add_like(self):
        ...
        client = APIClient()
        result = client.post('/locations/bookmarks/6/all_like/')
        bookmark = Bookmark.objects.annotate(num_likes=Count('likes')).get(id=6)
    ...
    @tag('unit_test')
    @patch('locations.view.Response')
    @patch('locations.view.BookmarkViewSet.get_object')
    @patch('locations.view.Like')
    def test_add_like(self, l_patch, go_patch, r_patch):
        ...
        self.assertEqual(l_patch.call.count, 1)
